...
AsCommander::~AsCommander() {
	...
	// Restore (Восстановите) the original active screen buffer.
...
bool AsCommander::Init() {
	...
	// Получите дескриптор (handle) стандартного (STDOUT) экранного буфера для копирования и создайте новый экранный буфер для копирования.
	Std_Input_Handle = GetStdHandle(STD_INPUT_HANDLE);
	Std_Output_Handle = GetStdHandle(STD_OUTPUT_HANDLE);
	//возвращаемое значение представляет собой дескриптор нового буфера экрана консоли
	...
	if (!GetConsoleScreenBufferInfo(Screen_Buffer_Handle, &Screen_Buffer_Info)) //Извлекает сведения об указанном буфере экрана консоли.
	{
