#include "Panel.h"
//--------------------------------------------------------------------------------------------------------------------------------------------
AFile_Descriptor::AFile_Descriptor(unsigned int attributes, unsigned int file_size_low, unsigned int file_size_higt, const wchar_t* file_name)
	: Attributes(attributes), File_Name(file_name)
{
	File_Size = ((unsigned long long)file_size_higt << 32) | (unsigned long long)file_size_low;
}
//--------------------------------------------------------------------------------------------------------------------------------------------
...
void APanel::Draw_Files() {	
	...
	for (auto *file : Files) {
		...
		if (y_offset >= Height - 5) {//дошли до конца первой колонки
			if (x_offset == 0) {
				x_offset += Width / 2;//переход на вторую колонку
				y_offset = 0;
			}
			else //дошли до конца второй колонки
				break;
		}
	}
}
